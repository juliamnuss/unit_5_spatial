...
and derived product algorithms available (look under Product Suites):
<https://www.earthdata.nasa.gov/centers/ob-daac/data-access-tools>
...
AQUA_MODIS.20020701_20250731.L3m.MC.CHL.chlor_a.9km.nc
...
-  `20250731`: end date in the format YYYYMMDD, so July 31, 2023
-  `.L3m`: Level 3 mapped 
...
```{r}
# Modis Aqua monthly climatology of chlor_a for July (Julian dates 182-212) averaged over 2002 - 2025
# https://oceandata.sci.gsfc.nasa.gov/
...
GOM_chl_map = ggplot() +
  ...
  coord_fixed(ratio = 1.3, xlim = lon_bounds, ylim = lat_bounds, expand=FALSE) + # crop map; 1.3 y/x aspect ratio (may want to use higher at the poles; use 1 near the equator)
...
```{r}
# For those of you focused on the Pacific, use "world2" from mapdata package, which shifts to a [0, 360] longitude system
pacific_map = map_data("world2") # note that world2Hires is buggy - creates some lines across the globe
chl_df_pacific_shift = chl_df %>%
  mutate(x = ifelse(x<0, x+360, x)) # manually adjusted longitudes in satellite raster to reflect [0, 360]
# head(pacific_map)
...
# If CRS didn't match up, we'd project the bath_m raster to match the CRS of the chl raster, then resample:
# bath_m_raster_chl_proj = raster::projectRaster(bath_m_raster, crs = crs(chl_GOM_raster))
# bath_layer_chl_dims = raster::resample(bath_m_raster_chl_proj, chl_GOM_raster) # resamples r1 extent, origin and resolution to that of r2
...
# What is the depth of waters classified as oligotrophic, mesotrophic and eutrophic?
summary(stack_df)
